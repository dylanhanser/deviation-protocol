...
class RunWorldVisitRow(Base):
    ...
    __table_args__ = (
        CheckConstraint("world_version BETWEEN 1 AND 9223372036854775807 AND region_version BETWEEN 1 AND 9223372036854775807 AND materialized_state_version IN (4, 5)", name="ck_run_world_visits_versions"),
        CheckConstraint("(visit_ordinal = 1 AND joined_state_version = 3 AND materialized_state_version = 4) OR (visit_ordinal = 2 AND joined_state_version = 4 AND materialized_state_version = 4) OR (visit_ordinal = 3 AND joined_state_version = 5 AND materialized_state_version = 5)", name="ck_run_world_visits_ordinal_join"),
...
class RunWorldPositionRow(Base):
    ...
    __table_args__ = (
        CheckConstraint("position_state_version IN (4, 5)", name="ck_run_world_positions_versions"),
...
class RunWorldVisitEntryRow(Base):
    ...
    __table_args__ = (
        ...
        ForeignKeyConstraint(("run_id", "continuous_story_line_id", "visit_id", "session_id"),
            ("run_world_visits.run_id", "run_world_visits.continuous_story_line_id",
             "run_world_visits.visit_id", "run_world_visits.session_id"),
            name="fk_run_world_visit_entries_visit", ondelete="RESTRICT", onupdate="RESTRICT"),
        ForeignKeyConstraint(("run_id", "continuous_story_line_id", "joined_state_version"),
            ("run_revisions.run_id", "run_revisions.continuous_story_line_id", "run_revisions.state_version"),
            name="fk_run_world_visit_entries_revision", ondelete="RESTRICT", onupdate="RESTRICT"),
        PLAYER_CHARACTER_TABLE_OPTIONS,
    )
    run_id: Mapped[str] = mapped_column(_ascii_varchar(128), primary_key=True)
    continuous_story_line_id: Mapped[str] = mapped_column(_ascii_varchar(128), nullable=False)
    ...
    session_id: Mapped[str] = mapped_column(_legacy_session_id_varchar(), nullable=False)
    joined_state_version: Mapped[int] = mapped_column(BigInteger, nullable=False)
    entry_schema: Mapped[str] = mapped_column(_ascii_varchar(64), nullable=False)
    entry_canonical: Mapped[bytes] = mapped_column(mysql.MEDIUMBLOB(), nullable=False)
    entry_sha256: Mapped[bytes] = mapped_column(mysql.BINARY(32), nullable=False)
    created_at: Mapped[datetime] = mapped_column(mysql.DATETIME(fsp=6), nullable=False)
