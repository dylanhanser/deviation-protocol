...
View 的 `scenario_status` 继续只表示 `ACTIVE/ENDED` 页面生命周期；稳定存在的 `ending_status` 在活动期为 `null`，并只从权威 runtime 状态投影结算后的 `RESOLVED/FAILED`。客户端不得从 ending ID、标题、摘要或叙事正文推断成败。
