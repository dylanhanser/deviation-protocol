...
`ScenarioDefinition.narrative_outcome_rules` is a closed, versioned vocabulary. Rules reference validated phase/NPC/fact/clue/decision IDs and existing StoryDirector action costs. Effect payloads are fixed server templates; no model value can become a fact, clue, clock delta, event payload, resource, inventory, NPC-existence change, FIXED fact, or anomaly effect. Catalog validation rejects unknown references, unreachable decision bindings, invalid fact effects, and ambiguous mutual-exclusion priorities. The first content pack adds only purposeful signalling, quiet observation, and constrained custom no-effect at the opening.

The provider sees opaque tokens rather than internal rule IDs/templates. `NarrativeOutcomePolicy` recomputes candidates from locked state and verifies the real narrative route, job/action/state/scenario binding, visible entities/NPCs, preconditions, result/prose consistency, and mutex rules. Prose consistency is deliberately conservative: it checks visible speakers plus data-defined required/forbidden terms, not unrestricted natural-language entailment. Dialogue and prose therefore never prove a world fact; the structured server event remains the only authority. Only the policy can create the capability consumed by `NarrativeEventIssuer`; the issuer rechecks job and lease plus all bindings and proposal digest, and derives `VALIDATED_NARRATIVE_OUTCOME` solely from server data. `StoryDirector` applies the sealed event on a deeply isolated candidate, so there is no second state-mutation system.
