"""R10: independent processes execute the complete public three-visit journey."""
import hashlib
import json
import os
...
def journey(seed,choice,profile):
    env=os.environ.copy()
    ...
    try:
        ...
        assert send("POST",f"/v1/sessions/{archive}/actions",dict(action_type="CHOOSE",choice_id=f"receipt_archive.action.{choice}",
        ...
        assert final["ending_status"]==("RESOLVED" if choice=="seal" else "FAILED")
        exited=send("POST",f"/v1/sessions/{archive}/run-exit",dict(expected_run_state_version=5,
            expected_session_state_version=final["metadata"]["state_version"]),"e2e.exit")
        assert exited["run_state_version"]==6
...
def test_r10_three_visit_public_replay_has_complete_cross_process_identity(choice,profile):
    first=journey(1,choice,profile)
    second=journey(9173,choice,profile)
    assert first==second
