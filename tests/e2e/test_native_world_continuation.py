"""C09: independent processes execute the complete public two-world journey."""
import hashlib
import json
import os
...
def journey(seed,choice,profile):
    ...
    try:
        ...
        assert send("POST",f"/v1/sessions/{sid}/actions",dict(turn_id="e2e.choice",client_request_id="e2e.choice",action_type="CHOOSE",
            decision_id=view["narrative_frame"]["decision_id"],choice_id=f"undelivered_receipt.action.{choice}"))["state_changed"]
        ...
        exited=send("POST",f"/v1/sessions/{sid}/run-exit",dict(expected_run_state_version=4,expected_session_state_version=final["metadata"]["state_version"]),"e2e.exit")
        assert exited["run_state_version"]==5
...
def test_two_world_public_replay_has_complete_cross_process_identity(choice,profile):
