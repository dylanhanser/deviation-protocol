...
def test_run_migration_matches_shared_metadata_and_is_linear(
    ...
    for table_name in RUN_TABLES:
        ...
        for revision in ("20260917_0008", "20260918_0009"):
    ...
    scripts = ScriptDirectory.from_config(Config(str(ROOT / "alembic.ini")))
    assert scripts.get_heads() == ["20260918_0009"]
...
def test_mysql_run_migration_upgrades_only_the_designated_test_database(
    ...
    command.upgrade(Config(str(ROOT / "alembic.ini")), "20260918_0009")
...
async def test_mysql_run_schema_is_exact_and_binding_seam_is_nullable(
    ...
    assert tables == set(RUN_TABLES) | {"run_protocol_bindings", "run_entry_world_bindings", "run_world_states", "run_world_visits", "run_world_positions"}
