...
def test_exact_preloaded_bundles_never_merge_or_fallback(monkeypatch):
    ...
    registry=SessionContentRegistry((a,b))
    assert SessionContentRegistry((b,a)).continuation_pool()==registry.continuation_pool()
...
def test_independent_approved_pack_pin_and_declared_values():
    ...
    assert SessionContentRegistry((bundle,)).continuation_pool()[0].content_sha256==DESTINATION_CONTENT_SHA256
