...
断线恢复使用 `GET /v1/sessions/{session_id}/view`。它从应用服务重新验证 snapshot 后聚合 metadata、安全 Frame、`PlayerVisibleStateProjection`、`PlayerMemoryProjection`、公开场景/时钟状态和仅 COMMITTED 的近期正文，不直接序列化 `GameState`。近期正文最多 6 条、总计最多 12,000 字符和 24,000 UTF-8 bytes；活动场景不返回 ending 字段。Narrative 请求可通过 `GET /v1/sessions/{session_id}/requests/{client_request_id}` 查询 PENDING、COMMITTED、STALE、OUTCOME_UNKNOWN 或 FAILED；PENDING 固定返回 `Retry-After: 2`，查询不 claim lease、不调用 Provider。遗留 `FAILED_RETRYABLE` 行也保守公开为 `FAILED/DO_NOT_RETRY`，公共协议不宣称支持重新提交。

确定性、有界的玩家长期记忆继续接入 session 创建、本地权威回合、Narrative finalize、只读玩家状态和安全 prompt。可信事件必须先在当前 UoW 中 insert/flush 并取得不可序列化的事务内 receipt，声明式 `MemoryRule` 才能生成并应用绑定 session/version/state 的 plan；event、memory、snapshot、response/job 和 session version仍只 commit 一次。外部 DeepSeek 调用发生在所有 UoW、`AsyncSession` 与 MySQL 行锁退出之后。
...
`narrative_jobs` is created by the independent `20260719_0003` migration with InnoDB/utf8mb4, MySQL JSON, session/client-request uniqueness, cascading session deletion, and session/status plus status/lease indexes. Its normal lifecycle is `PREPARED -> IN_PROGRESS -> PROPOSAL_VALIDATED -> COMMITTED`; production failure transitions use `FAILED_TERMINAL`, `STALE`, or `OUTCOME_UNKNOWN`. The internal `FAILED_RETRYABLE` enum remains only for legacy-row compatibility and has no production transition. Jobs persist only the bounded player-safe request, request/state fingerprints, scenario version, provider/model names, lease metadata, a validated-but-unaccepted proposal subset, stable error codes, and—only for COMMITTED jobs—accepted text. The proposal subset includes candidate prose for crash recovery. It is internal, non-authoritative, and never a player response or recent-context source. Terminal jobs may retain it until their job/session row is deleted. Jobs never persist credentials, Authorization headers, the full system prompt, raw provider envelopes, or chain-of-thought.

The deterministic final CHOOSE uses the same durable row only as an atomic settlement-text record: `prompt_schema_version=local-server-template-v1`, status COMMITTED, attempt count zero, no lease, and no Provider invocation. Its accepted text is copied exclusively from the trusted decision template and commits with the ending event, completed memory, snapshot, response, and version.
...
The model sees opaque per-turn outcome tokens and safe descriptions, never internal rule IDs or effect templates. `NarrativeOutcomePolicy` recomputes eligibility against locked authority; `NarrativeEventIssuer` can emit `VALIDATED_NARRATIVE_OUTCOME` only from server-defined templates. The scenario now includes bounded production templates for clinical recheck, records, audit, patient observation, and phase-safe no-effect fallbacks. Fixed templates—not prose—grant clues, open or enter locations, identify the concrete runtime NPC that acknowledges the player is alive, and charge declared time. Investigation clue authority requires a server-opened location, the authoritative current location, and an ordinary mechanical `OBSERVE`/`EXPLORE` action; the first records-room entry comes from a bound public decision. Free text, including action-shaped hidden-truth guesses, cannot satisfy that structural gate and receives fixed safe no-effect text. Every production outcome publishes fixed server prose, so contradictory Provider wording cannot become the accepted response. Proposal validation still rejects internal rule/job/lease/receipt/provider/model markers, future ending IDs, opaque outcome tokens, and other internal identifier shapes. Only sealed server events are authoritative.
...
首个正式结构化内容包位于 `config/scenarios/death_certificate_v1.json`，规格见 `docs/scenarios/death_certificate_v1.md`。仓库只包含原创的结构化设计，没有参考小说原文或改写文本。Phase 2.4b 的声明式 outcome 目录覆盖开局、临床复核、三条调查生产路线和各自由行动阶段的安全 no-effect fallback。
